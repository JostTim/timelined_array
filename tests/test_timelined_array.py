...
from timelined_array import MaskedTimelinedArray, TimelinedArray
...
def timelined_array_3D():
    return TimelinedArray(
        np.random.rand(25, 50, 75), timeline=np.arange(50), time_dimension=1
    )
...
def axis_affecting_functions_blueprint(
    timelined_array_3D, func, affected_axis, time_dimension, shape
):
    result = func(timelined_array_3D, axis=affected_axis)
    if time_dimension is None:
        with pytest.raises(AttributeError):
            result.time_dimension  # noqa: B018
    else:
        assert result.time_dimension == time_dimension

    if shape is None:
        with pytest.raises(AttributeError):
            result.shape  # noqa: B018
    else:
        assert result.shape == shape


@pytest.mark.parametrize(
    "affected_axis, time_dimension, shape", get_axis_parameters_3D()
)
def test_ta_mean(timelined_array_3D, affected_axis, time_dimension, shape):
    axis_affecting_functions_blueprint(
        timelined_array_3D, np.mean, affected_axis, time_dimension, shape
    )
...
def test_ta_sum(timelined_array_3D, affected_axis, time_dimension, shape):
    axis_affecting_functions_blueprint(
        timelined_array_3D, np.sum, affected_axis, time_dimension, shape
    )
...
def test_ta_std(timelined_array_3D, affected_axis, time_dimension, shape):
    axis_affecting_functions_blueprint(
        timelined_array_3D, np.std, affected_axis, time_dimension, shape
    )
...
def test_ta_var(timelined_array_3D, affected_axis, time_dimension, shape):
    axis_affecting_functions_blueprint(
        timelined_array_3D, np.var, affected_axis, time_dimension, shape
    )
